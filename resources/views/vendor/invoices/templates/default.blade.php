            ...
            <tbody>
                <tr>
                    ...
                    <td class="border-0 pl-0">
                        ...
                        @endif
                        <p>Mã đơn hàng <strong>{{ $invoice->getSerialNumber() }}</strong></p>
                        <p>Ngày đặt hàng: <strong>{{ $invoice->getDate() }}</strong></p>
                    </td>
                </tr>
            </tbody>
        </table>
                ...
                <tr>
                    <td class="px-0">
                        ...
                        @if($invoice->seller->vat)
                            <p class="seller-vat">
                                MST: {{ $invoice->seller->vat }}
                            </p>
                        @endif

                        @if($invoice->seller->phone)
                            <p class="seller-phone">
                                Số điện thoại: {{ $invoice->seller->phone }}
                            </p>
                        @endif
                    </td>
                    ...
                    <td class="px-0">
                        ...
                        @if($invoice->buyer->address)
                            <p class="buyer-address">
                                Địa chir: {{ $invoice->buyer->address }}
                            </p>
                        @endif
                        ...
                        @if($invoice->buyer->phone)
                            <p class="buyer-phone">
                                Số điện thoại: {{ $invoice->buyer->phone }}
                            </p>
                        @endif
        ...
        <table class="table table-items">
            <thead>
                <tr>
                    ...
                    <th scope="col" class="text-right border-0">Đơn giá</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-right">
                        {{ number_format($item->price_per_unit, 0, '', ',') }} đ
                    </td>
                    @if($invoice->hasItemDiscount)
                        <td class="text-right">
                    ...
                    <td class="text-right pr-0">
                        {{ number_format($item->sub_total_price, 0, '', ',') }} đ
                    </td>
                </tr>
                @endforeach
                {{-- Summary --}}
                ...
                @if($invoice->shipping_amount)
                    <tr>
                        ...
                        <td class="text-right pr-0">
                            {{ number_format($invoice->shipping_amount, 0, '', ',') }} đ
                        </td>
                    </tr>
                @endif
                    <tr>
...
{{--            {{ trans('invoices::invoice.amount_in_words') }}: {{ $invoice->getTotalAmountInWords() }}--}}
        ...
        <p>
            Vui lòng thanh toán trước ngày: {{ $invoice->getPayUntilDate() }}
        </p>
